...
import io.flutter.plugins.webviewflutter.filechooser.FileChooser;
...
public class WebViewFlutterPlugin implements FlutterPlugin, ActivityAware {
  ...
  public void onAttachedToActivity(@NonNull ActivityPluginBinding activityPluginBinding) {
    ...
    fileChooser = new FileChooser(activityPluginBinding.getActivity());
    activityPluginBinding.addActivityResultListener(new PluginRegistry.ActivityResultListener() {
      @Override
      public boolean onActivityResult(int i, int i1, Intent intent) {
        if(fileChooser != null && fileChooser.getResultHandler() != null){
          return fileChooser.getResultHandler().handleResult(i,i1,intent);
        }
        return false;
      }
    });
